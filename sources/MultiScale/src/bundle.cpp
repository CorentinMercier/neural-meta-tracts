...
//    Bundle removedFibers;
	...
	if (reduceNbFibers)
	{
		...
		for (unsigned int lineCount = 0; lineCount<numberOfFibers*divisionValue; lineCount++)
		{
			...
			{
				std::vector<glm::vec3> subvector(Points.begin() + temp, Points.begin() + temp + lineCount - 1);
				Fiber newFib(subvector, NumberPointsPerFiber(lineCount,0));
...
void Bundle::loadTckFibers(const string & filename, bool reduceNbFibers)
{
	...
	while (!isinf(newPoint))
	{
		...
		if (isnan(newPoint) || isinf(newPoint))
		{
			...
			{
				std::vector<glm::vec3> Points(newFiber.size());
...
vector<vector<glm::vec3> > Bundle::getFibers()
{
	vector<vector<glm::vec3> > fibres(m_fibers.size());
...
#if (TEST_FIBERS_VALIDITY)
	{
		...
		while (!same && i<m_fibers.size())
		{
			...
			if (!same && (new_fiber[0]==m_fibers[i][0] || new_fiber[new_fiber.size()-1]==m_fibers[i][m_fibers[i].size()-1]))
			{
				...
				new_fiber.moveExtremities(glm::vec3(ecart, ecart, ecart));
			}
			i++;
		}
		if (same)
...
void Bundle::computeFiberFuzzy(unsigned int i)
{
	...
	cylinderValue += glm::length(m_fibers[i].getProfileTransform(1).center - m_fibers[i].getProfileTransform(0).center) / m_fibers[i].cylinderLength() * ptValue;
	ptValue = m_fuzzySet->getValueFromSegment(m_fibers[i][0], m_fibers[i][1]);
	value += glm::length(m_fibers[i][1] - m_fibers[i][0]) / m_fibers[i].length() * ptValue;
#pragma omp parallel for reduction (+:value, cylinderValue)
	for (unsigned int j=1; j<m_fibers[i].size()-2; j++)
	{
		...
		value += glm::length(m_fibers[i][j + 1] - m_fibers[i][j]) / m_fibers[i].length() * ptValueFiber;
		...
		cylinderValue += glm::length(m_fibers[i].getProfileTransform(j + 1).center - m_fibers[i].getProfileTransform(j).center) / m_fibers[i].cylinderLength() * ptValueCylinder;
	}
	ptValue = m_fuzzySet->getValueFromSegment(m_fibers[i][m_fibers[i].size()-2], m_fibers[i][m_fibers[i].size()-1]);
	value += glm::length(m_fibers[i][m_fibers[i].size() - 1] - m_fibers[i][m_fibers[i].size() - 2]) / m_fibers[i].length() * ptValue;
	...
	cylinderValue += glm::length(m_fibers[i].getProfileTransform(m_fibers[i].size() - 1).center - m_fibers[i].getProfileTransform(m_fibers[i].size() - 2).center) / m_fibers[i].cylinderLength() * ptValue;
...
void Bundle::computeFiberFuzzyNotParallel(unsigned int i)
{
	...
	for (unsigned int j=0; j<m_fibers[i].size()-1; j++)
	{
		...
		value += glm::length(m_fibers[i][j + 1] - m_fibers[i][j]) / m_fibers[i].length() * ptValue;
	}
	...
	value=0;
	float ptValue = m_fuzzySet->getValueFromVolume(m_fibers[i].getProfileTransform(0).center, m_fibers[i].getProfileTransform(0), m_fibers[i].getProfileTransform(1), m_fibers[i].getProfileTransform(2).center);
	value += glm::length(m_fibers[i].getProfileTransform(1).center - m_fibers[i].getProfileTransform(0).center) / m_fibers[i].cylinderLength() * ptValue;
	for (unsigned int j=1; j<m_fibers[i].size()-2; j++)
	{
		...
		value += glm::length(m_fibers[i].getProfileTransform(j+1).center - m_fibers[i].getProfileTransform(j).center) / m_fibers[i].cylinderLength() * ptValue;
	}
	...
	value*=extremity;
	ptValue = m_fuzzySet->getValueFromVolume(m_fibers[i].getProfileTransform(m_fibers[i].size()-3).center, m_fibers[i].getProfileTransform(m_fibers[i].size()-2), m_fibers[i].getProfileTransform(m_fibers[i].size()-1), m_fibers[i].getProfileTransform(m_fibers[i].size()-1).center);
	value += glm::length(m_fibers[i].getProfileTransform(m_fibers[i].size() - 1).center - m_fibers[i].getProfileTransform(m_fibers[i].size() - 2).center) / m_fibers[i].cylinderLength() * ptValue;
	...
	for (unsigned int i=0; i<size; i++)
	{
		for (set<unsigned int>::iterator ite=m_fibers[i].m_neighbours.begin(); ite!=m_fibers[i].m_neighbours.end(); ++ite)
		{
			...
			if (i<j)//To compute each pair only once
			{
				paire a;
				a.fib1=i;
				a.dist=m_metric->metric(m_fibers[i], m_fibers[j]);
				a.fib2=j;
...
#pragma omp parallel for
		for (unsigned int i=0; i<nbNeighbours; i++)
		{
			...
			{
				paire a;
				a.dist=m_metric->metric(m_fibers[nbFibres-1], m_fibers[j]);
				a.fib1=nbFibres-1;
				a.fib2=j;
#if (STATS)
...
vector<glm::vec3> Bundle::drawNeighbors()
{
...
vector<glm::vec3> Bundle::drawOneNeighbour()
{
	vector<glm::vec3> edges;
	unsigned int i=(float)rand()/RAND_MAX*(float)getCurrentNberOfFibers();
	unsigned int j=0;
