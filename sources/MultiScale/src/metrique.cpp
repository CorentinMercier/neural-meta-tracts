...
float fiberLength(const std::vector<glm::vec3> &fiber, int size)
{
	float length=0;
	for (int i=0; i<size-1; i++)
		length += glm::length2(fiber[i + 1] - fiber[i]);
	return length;
}
...
float Metrique::MC(Fiber &fiber1, Fiber &fiber2)
{
	...
	for (unsigned int i=0; i<fiber1.size(); i++)
		...
		{
			distances(i,j)= glm::length2(fiber1[i] - fiber2[j]);
...
float Metrique::averageOfPointwiseEuclideanMetric(Fiber &fiber1, Fiber &fiber2)
{
	...
	float sum=0;
	for (unsigned int i=0; i<fiber1.size(); i++)
		sum += glm::length2(fiber1[i] - fiber2[i]);
	sum /= fiber1.size();
	return sum;
}
...
float Metrique::MDF(Fiber &fiber1, Fiber &fiber2)
{
	...
	{
		ddirect += glm::length2(fiber1[i] - fiber2[i]);
		dflipped+= glm::length2(fiber1[i] - fiber2[size-i]);
	}
	ddirect/=(float)size;
	dflipped/=(float)size;
...
float scalarProductPDM(Fiber &fiber1, Fiber &fiber2, float sigma)
{
	...
	for (unsigned int i=0; i<fiber1.size(); i++)
		for (unsigned int j=0; j<fiber2.size(); j++)
			scalarProduct += exp(-glm::length2(fiber1[i] - fiber2[j]) / (sigma * sigma)) / denominator;
...
float scalarProductVD(Fiber &fiber1, Fiber &fiber2, float sigma)
{
	...
	for (unsigned int i=0; i<fiber1.size()-1; i++)
	{
		...
		{
			ksigma = exp(-glm::length2(fiber1.getCenter(i) - fiber2.getCenter(j)) / (sigma * sigma));
			kn = glm::dot(fiber1.getTangent(i), fiber2.getTangent(j));
			scalarProduct += ksigma * kn * kn / (glm::length(fiber1.getTangent(i))*glm::length(fiber2.getTangent(j)));
...
float scalarProductWeightedCurrents(Fiber &fiber1, Fiber &fiber2, float lambdac, float lambdab, float lambdag)
{
	...
	for (unsigned int i=0; i<fiber1.size()-1; i++)
	{
		for (unsigned int j=0; j<fiber2.size()-1; j++)
		{
			kg = exp(-glm::length2(fiber1.getCenter(i) - fiber2.getCenter(j)) / (lambdag * lambdag));
			scalarProduct += glm::dot(fiber1.getTangent(i), kg * fiber2.getTangent(j));
		}
	}
	kc = exp(-glm::length2(fiber1[0] - fiber2[0]) / (lambdac * lambdac));
	kb = exp(-glm::length2(fiber1[fiber1.size() - 1] - fiber2[fiber2.size() - 1]) / (lambdab * lambdab));
	scalarProduct *= kc * kb;
	return scalarProduct;
}
...
float Metrique::metric(Fiber fiber1, Fiber fiber2)
{
...
{
	return glm::length2(fiber1[0] - fiber2[0]) + glm::length2(fiber1[fiber1.size() - 1] - fiber2[fiber2.size() - 1]);
}
...
Fiber Metrique::mergeOfFibers(Fiber& fiber1, Fiber& fiber2)
{
	...
	float dist1sens1 = glm::length(fiber1[0] - fiber2[0]);
	float dist2sens1 = dist1sens1;
	float dist1sens2 = glm::length(fiber1[extremite1sens2] - fiber2[extremite2sens2]);
	float dist2sens2 = dist1sens2;
	for (unsigned int i=1; i<min(fiber1.size(), fiber2.size()); i++)
	{
		float temp = glm::length(fiber1[i] - fiber2[0]);
		if (temp < dist1sens1)
		{
			dist1sens1 = temp;
			extremite1sens1 = i;
		}
		temp = glm::length(fiber1[0] - fiber2[i]);
		if (temp < dist2sens1)
		{
			dist2sens1=temp;
			extremite2sens1=i;
		}
		temp = glm::length(fiber1[fiber1.size() - 1 - i] - fiber2[fiber2.size() - 1]);
		if (temp<dist1sens2)
		{
			dist1sens2=temp;
			extremite1sens2=fiber1.size()-1-i;
		}
		temp = glm::length(fiber1[fiber1.size() - 1] - fiber2[fiber2.size() - 1 - i]);
		if (temp<dist2sens2)
		{
			dist2sens2=temp;
...
//        unsigned int extremitesens1, extremitesens2;
	...
	for (unsigned int i=1; i<extremite1sens1; i++)
	{
		...
		if (extremite1sens1==extremite1sens2)
			pt1=-1;
		scalarProduct1 = 1 - fabs(glm::dot(glm::normalize(fiber1[extremite1sens1 + pt1] - fiber1[extremite1sens1]), glm::normalize(fiber2[extremite2sens1] - fiber1[extremite1sens1])));
//        scalarProduct1=1-fabs((fiber1[extremite1sens1+pt1]-fiber1[extremite1sens1]).normalized().dot((nvPoints[nvPoints.size()-1]-fiber1[extremite1sens1]).normalized()));
		if (extremite2sens1 == extremite2sens2)
			pt2 = -1;
		scalarProduct2 = 1 - fabs(glm::dot(glm::normalize(fiber2[extremite2sens1 + pt2] - fiber2[extremite2sens1]), glm::normalize(fiber1[extremite1sens1] - fiber2[extremite2sens1])));
//        scalarProduct2=1-fabs((fiber2[extremite2sens1+pt2]-fiber2[extremite2sens1]).normalized().dot((nvPoints[nvPoints.size()-1]-fiber2[extremite2sens1]).normalized()));
		...
		width.push_back(max((fiber1.getWidth(extremite1sens1) * factor1 * scalarProduct1 + fiber2.getWidth(extremite2sens1) * factor2 * scalarProduct2 + glm::length(fiber1[extremite1sens1] - fiber2[extremite2sens1]) * scalarProduct3) / 2.f, max(fiber1.getWidth(extremite1sens1) * scalarProduct1, fiber2.getWidth(extremite2sens1) * scalarProduct2)));
		//width.push_back((fiber2.getWidth(extremite2sens1)+glm::vec3(fiber2[extremite2sens1]-nvPoints[nvPoints.size()-1]).norm())*scalarProduct2);
...
//        scalarProduct1=1-fabs((fiber1[extremite1sens1+pt1]-fiber1[extremite1sens1]).normalized().dot((nvPoints[nvPoints.size()-1]-fiber1[extremite1sens1]).normalized()));
		...
		scalarProduct3 = 1;//fabs((fiber1.getTangent(extremite1sens1)-fiber2.getTangent(extremite2sens1)).normalized().dot((nvPoints[nvPoints.size()-1]-(nvPoints[nvPoints.size()-2])).normalized()));
		width.push_back(max((fiber1.getWidth(extremite1sens1) * factor1 * scalarProduct1 + fiber2.getWidth(extremite2sens1) * factor2 * scalarProduct2 + glm::length(fiber1[extremite1sens1] - fiber2[extremite2sens1]) * scalarProduct3) / 2.f, max(fiber1.getWidth(extremite1sens1) * scalarProduct1, fiber2.getWidth(extremite2sens1) * scalarProduct2)));
		//width.push_back((fiber1.getWidth(extremite1sens1)+glm::vec3(fiber1[extremite1sens1]-nvPoints[nvPoints.size()-1]).norm())*scalarProduct1);
...
//        scalarProduct1=fabs((fiber1[extremite1sens1+pt1]-fiber1[extremite1sens1]).normalized().dot((nvPoints[nvPoints.size()-1]-(nvPoints[nvPoints.size()-2])).normalized()));
	...
	std::vector<glm::vec3> points(nvPoints.size());
...
Transfo computeNewTransfo(Fiber& fiber1, unsigned int positionFib1, Fiber& fiber2, unsigned int positionFib2)
{
	...
	Transfo newTransfo;
	glm::vec3 direction = fiber1[positionFib1]-fiber2[positionFib2];
	float distBetweenEllipses = glm::length(direction);
	direction = glm::normalize(direction);
	newTransfo.center = (fiber1[positionFib1] + fiber2[positionFib2]) / 2.f;
	newTransfo.rotation = glm::normalize((fiber1.getProfileTransform(positionFib1).rotation + fiber2.getProfileTransform(positionFib2).rotation) / 2.f);
...
//    float value2=fiber2.getProfileTransform(positionFib2).a*fabs(fiber2.getProfileTransform(positionFib2).ellipseOrientation.dot(direction.normalized()));
	...
	//Repère de l'ellipse
	float p2 = pow(tan(acos(min(1.f, fabs(glm::dot(glm::normalize(glm::cross(fiber1.getProfileTransform(positionFib1).ellipseOrientation, fiber1.getProfileTransform(positionFib1).rotation)), direction))))), 2.f);
	//cout << p2 << " " << sqrt(p2) << " ";
	...
	p2 = pow(tan(acos(min(1.f, fabs(glm::dot(glm::normalize(glm::cross(fiber2.getProfileTransform(positionFib2).ellipseOrientation, fiber2.getProfileTransform(positionFib2).rotation)), direction))))), 2.f);
	...
	newTransfo.center += direction * (value1 - value2) / 4.f;
	...
	p2 = pow(tan(acos(min(1.f, fabs(glm::dot(fiber1.getProfileTransform(positionFib1).ellipseOrientation, direction)))))/**fiber1.getProfileTransform(positionFib1).a*/, 2.f);
	...
	p2 = pow(tan(acos(min(1.f, fabs(glm::dot(fiber2.getProfileTransform(positionFib2).ellipseOrientation, direction)))))/**fiber2.getProfileTransform(positionFib2).a*/, 2.f);
	...
	if (newTransfo.a==axe2)
		newTransfo.ellipseOrientation = glm::normalize(glm::cross(glm::normalize(direction), newTransfo.rotation));
	else
		newTransfo.ellipseOrientation = glm::normalize(direction);
	return newTransfo;
}
...
//    }
	...
	while(extremite1sens1<(extremite1sens2) && extremite2sens1<(extremite2sens2))
	{
		...
		if (fibre1Unitaire)
		{
			...
			extremite1sens1=cumul1;
			cumul2+=pas;
			float dist = glm::length(fiber1[extremite1sens1] - fiber2[cumul2]);
			extremite2sens1=cumul2;
			for (int j=-20; j<30; j++)
			{
				if (cumul2+j>=0 && cumul2+j<fiber2.size())
				{
					float distTemp = glm::length(fiber1[extremite1sens1] - fiber2[cumul2+j]);
					if (distTemp<dist)
					{
		...
		else
		{
			...
			extremite2sens1=cumul2;
			cumul1+=pas;
			float dist = glm::length(fiber1[cumul1] - fiber2[extremite2sens1]);
			extremite1sens1=cumul1;
			for (int j=-20; j<30; j++)
			{
				if (cumul1+j>=0 && cumul1+j<fiber1.size())
				{
					float distTemp = glm::length(fiber1[cumul1 + j] - fiber2[extremite2sens1]);
					if (distTemp<dist)
					{
...
//        }
	...
	glm::vec3 lastOrientation=profileTransform[0].ellipseOrientation;
	for (unsigned int i=0; i<profileTransform.size(); i++)
	{
		Matrix3f matrice;
		if (profileTransform[i].ellipseOrientation.dot(lastOrientation)<0)
			...
			lastOrientation=profileTransform[i].ellipseOrientation;
		glm::vec3 axeY=profileTransform[i].rotation.cross(lastOrientation);
