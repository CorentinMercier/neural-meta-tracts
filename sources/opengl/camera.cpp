...
void camera::computeModelview()
{
	...
	m_normal = glm::transpose(glm::inverse(m_modelview));
}
...
void camera::rotation(int const x, int const y)
{
	...
	if(sqrt(pow(x0-x1,2)+pow(y0-y1,2)>1e-6))
	{
		...
		m_quat = glm::angleAxis(phi, axis) * m_quat;// m_quat=AngleAxisf(phi, axis)*m_quat;
	}
	m_xPrevious=x;
	m_yPrevious=y;
}
...
void camera::rotateAlongY(float x)
{
	...
	//compute quaternion to apply
	m_quat = glm::angleAxis(phi, glm::vec3(1.0f)) * m_quat;
}
